...
| **The detectors asserted instead of measuring** | `tool-boundary-corpus` — 18 labelled cases (13 tool-list, 5 code) and a detector-agnostic harness. Measured: mcpaudit **P=1.000 R=1.000**, agentbound **P=0.750 R=1.000**. The corpus immediately found a real false positive in agentbound (a pattern inside a string literal, which its comment fix does not cover) — kept failing and named in CI rather than deleted. Two of my fixtures were wrong, not the detector: the rules require the framework to *define* the tool, and the full signature→params→filter relation. **A fix to make:** agentbound's string-literal false positive; the corpus will then measure precision 1.000 and its CI will go red until the known-failure entry is removed. |
